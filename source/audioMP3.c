...
// Fills the buffer with all zero values to generate silence.
// Not the most elegant solution.
void empty_mp3_buffer( int buffer_number ) {
   int counter;

   for ( counter = 0 ; counter < AUDIO_LOCAL_BUFFER_SIZE; counter++ ) {
	   audio_read_buffer[ buffer_read * AUDIO_LOCAL_BUFFER_SIZE + counter ] = 0;
   }

   audio_buffer_length[ buffer_number ] = AUDIO_LOCAL_BUFFER_SIZE/2;
   return;
		...
		if (err) {
			...
			if (mp3FrameInfo.nChans == 1) {
				for(i = mp3FrameInfo.outputSamps-1;i >= 0;i--) 	{
					audio_read_buffer[ buffer_number * AUDIO_LOCAL_BUFFER_SIZE + 2 * i]     = audio_read_buffer[ buffer_number * AUDIO_LOCAL_BUFFER_SIZE + i];
					audio_read_buffer[ buffer_number * AUDIO_LOCAL_BUFFER_SIZE + 2 * i + 1] = audio_read_buffer[ buffer_number * AUDIO_LOCAL_BUFFER_SIZE + i];
				}
...
static void play_mp3(char* filename) {
	...
	if (FR_OK == f_open(&file, filename, FA_OPEN_EXISTING | FA_READ)) {
		...
		for (cc = 0 ; cc < NUMBER_BUFFERS-1 ; cc++ ) {
			if ( out_of_data ) {
							break;
			}
		...
		buffer_read = 0;
		buffer_write = NUMBER_BUFFERS-1;

/////////////////////////////////////////////////////////////////////////////////////////
		// Play mp3
		running_player = true;
		// Sets the audio playback settings.
		InitializeAudio(Audio44100HzSettings);

		// Starts the DAC DMA routine.
